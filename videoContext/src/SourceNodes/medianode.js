...
* @Date:   2018-11-05 11:01:16
* @Last Modified by:   Mayde
* @Last Modified time: 2018-11-09 14:18:43
*/
...
class MediaNode extends SourceNode {
  ...
  _load() {
  		...
  		if(this._element.readyState > 3 && !this._element.seeking) {
  			if(this._loopElement === false) {
  				if(this._stopTime === Infinity || this._stopTime == undefined) {
  					...
  					this._triggerCallback("durationchange", this.duration);
  				}
  			}
  			if(this._ready !== true) {
  	...
  	if(this._isResponsibleForElementLifeCycle) {
  		...
  		for(let key in this._attributes) {
  			this._element[key] = this.attributes[key];
  		}
  	}
  	if(this._element) {
  ...
  _update(currentTime, triggerTextureUpdate = true) {
  	...
  	if(this._startTime - this._currentTime <= this._preloadTime && 
